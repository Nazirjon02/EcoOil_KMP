...
class TransactionsScreenParent : Screen {
    @OptIn(ExperimentalMaterial3Api::class)
    @Composable
    override fun Content() {
        val client = MainRootScreen.LocalClient.current
        val viewModel: TransactionsViewModel = viewModel() // Получаем viewModel
        val transactions = remember { viewModel.transactions }

        // Загружаем транзакции, если они еще не загружены
        LaunchedEffect(Unit) {
            viewModel.loadTransactions(client)
        }

        TransactionsScreen(client, transactions)
    }
}
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun TransactionsScreen(client: InsultCensorClient?, transactions: List<TransactionDto>) {
    val groupedTransactions = remember(transactions) { groupTransactionsByDate(transactions) }
...
fun groupTransactionsByDate(
    ...
    return list
        .sortedByDescending { it.dateNoSeconds() }  // Сортируем по дате
        .groupBy { tx ->
            tx.dateNoSeconds().dateOnly() // Преобразуем в строку без времени
        }
}
